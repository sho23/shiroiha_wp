jQuery(document).ready(function($){
	...
	// カルーセル 投稿タイプ変更
	$('#contents_builder').on('change', '.cb_content-carousel .cb_post_type-radios :radio', function(){
		var $container = $(this).closest('.cb_content-carousel');
		$container.find('div[class*="cb_post_type-content-"]').hide();
		$container.find('.cb_post_type-content-' + this.value).show();
	});
	$('#contents_builder .cb_content-carousel .cb_post_type-radios :radio:checked').trigger('change');

	// カテゴリーリスト タブ
	$('#contents_builder').on('click', '.cb_content-category_list .categorydiv .category-tabs a', function(){
		if ($(this).closest('li').hasClass('tabs')) return false;
		$(this).closest('li').addClass('tabs').siblings().removeClass('tabs');
		var $container = $(this).closest('.categorydiv');
		$container.find('.tabs-panel').hide();
