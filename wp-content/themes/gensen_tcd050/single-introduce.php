  ...
  <div class="custom-header">
    <div class="cs-header-text">
      ...
      <?php if ($dp_options['show_shoulder_copy_introduce'] && get_post_meta($post->ID, 'shoulder_copy', true)) { ?>
        <?php $data = trim(get_post_meta($post->ID, 'shoulder_copy', true));
              $elms=explode(',',$data); ?>
        <p class="elm-inline"><i class="fas fa-map-marker-alt"></i> <?php echo $elms[0]; ?></p>
        <ul class="elm-inline">
    ...
    <div class="cs-header-btns">
      <ul>
        ...
        <li class="btn btn-blue elm-inline"><a href="#test-popup" onClick="ga('send', 'event', 'Link-<?php the_ID();?>', 'click', '#test-popup');" class="open-popup-link" id="ga-pop-link-<?php the_ID(); ?>"><p class="btn-icon"><i class="fas fa-calendar-alt fa-2x"></i></p><p>予約</p></a></li>
...
<?php
      ...
      if ($dp_options['show_related_introduce']) :
        $args = array('post_type' => $dp_options['introduce_slug'], 'post__not_in' => array($post->ID), 'posts_per_page' => $dp_options['related_post_num'], 'orderby' => 'rand');
